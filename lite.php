...
class DBT {
	...
	public function exec($sql){
		return $this->_cnx->exec((string)$sql);
	}
...
class ED {
	...
	public function menu($db='',$tb='',$left='',$sp=[]){
		...
		if($nr!=false){
		$rs=$r_ts[0];
		$sl2.="<option value='".$this->path.($r_ts[1]=='trigger'?"41/$db/$rs/".$r_ts[1]:(in_array($this->sg[0],$in)?$this->sg[0]:20)."/$db/".$rs)."'".($rs==$tb || ($c_sp >1 && $rs==$sp[1])?" selected":"").">$rs</option>";
		$qtype=$r_ts[1];
		}
		}
		if($qtype !='') $sl2.='</optgroup>';
...
case "5"://show tables
	...
	foreach($q_tabs as $r_tabs){
		...
		$vl="/$db/".$r_tabs[0];
		if($r_tabs[1]=="view"){
		$lk="40{$vl}/view"; $vdel="49{$vl}/view";
		} else {
		$lk="10{$vl}"; $vdel="26{$vl}";
		}
		echo "<tr class='r c$bg'><td>".$r_tabs[0]."</td><td>$q_num</td><td><a href='{$ed->path}{$lk}'>Structure</a><a class='del' href='{$ed->path}{$vdel}'>Drop</a>".(is_numeric($nr) || ($nr!=false && $q_num=="view") ? "<a href='{$ed->path}20/$db/{$r_tabs[0]}'>Browse</a>":"")."</td></tr>";
		}
	}
	echo "</table>";
...
case "6"://create table
	...
	if($q_ch) $ed->redir("5/$db",['err'=>"Name already exists"]);
		...
		echo $ed->form("6/$db")."<input type='hidden' name='ctab' value='".$ed->post('ctab')."'/><input type='hidden' name='nrf' value='".$ed->post('nrf')."'/>$stru";
		$nr=$ed->post('nrf');
		$i=0;
		while($i<$nr){
...
case "9":
	...
	if($ed->post('rtab','!e')){//rename table
		...
		$ed->con->exec("PRAGMA legacy_alter_table=on");
		$ren_tb=$ed->con->exec("ALTER TABLE $tb RENAME TO $new");
		$ed->con->exec("PRAGMA legacy_alter_table=off");
...
case "10"://structure
	...
	echo "</tbody><tfoot><tr><td class='auto' colspan='7'><div class='left'><button type='submit' name='primary'>Primary</button><button type='submit' name='index'>Index</button><button type='submit' name='unique'>Unique</button></div><div class='right link'><a href='{$ed->path}27/$db/$tb/analyze'>Analyze</a></div></td></tr></tfoot></table></form>";
...
case "20"://table browse
	...
	foreach($r_col as $row){
		...
		$s=$row[$key[$j]]?$row[$key[$j]]:'';
		$le=strlen($s);
		echo "blob: ".($le>4 ? "<a href='{$ed->path}33/$db/$tb/$nu/{$key[$j]}'>".number_format(($le/1024),2)." KB</a>":"0.00 KB");
...
case "21"://insert row
	...
	if($ed->post('save','i') || $ed->post('save2','i')){
		...
		foreach($q_pra as $r_pra){
			...
			foreach($bbs as $kj=>$bb) echo "<input type='radio' name='r{$r_pra['cid']}[]' value='$kj'/> $bb ";
	...
	if($ed->post('edit','i')){
		...
		foreach($q_rd as $r_rd){
			...
			if($stype=="blob"){
				if($ed->post('r'.$f_rd,'i')){
				$qr.=$f_rd."='',";
				}elseif(!empty($_FILES['d'.$f_rd]['tmp_name'])){
		...
		foreach($q_rd as $r_ed){
			...
			if($typ=="boolean"){
			foreach($bbs as $kk=>$bb) echo "<input type='radio' name='d{$nr}[]' value='$kk'".($arr[0][$nr]==$kk ? " checked":"")."/> $bb ";
			} elseif($typ=="blob"){
			$s=$arr[0][$nr]?$arr[0][$nr]:'';
			echo number_format((strlen($s)/1024),2)." KB<i class='right'><input type='checkbox' name='r{$nr}'/> reset</i><br/><input type='file' name='d{$nr}'/>";
			} elseif($typ=="text"){
		...
		echo "<tr><td><a class='del right link' href='{$ed->path}23/$db/$tb/$nu/".($id==""?"isnull":base64_encode($id)).(!empty($nu1) && !empty($id1)?"/$nu1/".base64_encode($id1):"")."'>Delete</a></td><td><button type='submit' name='edit'>Update</button></td></tr></table></form>";
	}
break;
...
case "24"://search
	...
	if(!empty($ed->sg[3]) && $ed->sg[3]=='reset') $ed->redir("20/$db/$tb",['ok'=>"Reset search"]);
...
case "27"://analyze
	...
	$ed->con->exec("$op $tb");
	$msg=['ok'=>"Successfully $op runned"];
	} else $msg=['err'=>"Action $op failed"];
	$ed->redir("10/$db/$tb",$msg);
break;
...
case "30"://import
	...
	if($ed->post('qtxt','!e')){//run text
		...
		$ed->redir("5/$db",['err'=>"No file to upload"]);
			...
			if($fext[2]=='sql'){
				...
				$e=[array_merge(["DATABASE ".$fext[1]],preg_split($rgex,$fi,-1,PREG_SPLIT_NO_EMPTY))];
			} elseif($fext[2]=='csv'){
				...
				$e=$ed->imp_xml($tmp);
			} elseif(in_array($fext[2],['db',substr($ed->ext,1)])){
				...
				if(@function_exists('gzopen')){
					...
					gzclose($gzfile);
					if($ex[2]=='sql') $e=[array_merge(["DATABASE ".$ex[1]],preg_split($rgex,$ed->utf($e),-1,PREG_SPLIT_NO_EMPTY))];
					...
					elseif($ex[2]=='tar'){
						...
						if($fx[2]=='sql') $e[]=[array_merge(["DATABASE ".$fx[1]],preg_split($rgex,$ed->utf($f_b),-1,PREG_SPLIT_NO_EMPTY))];
				...
				if($res === TRUE){
					...
					if(!empty($zn[2])){
					if($zn[2]=='sql') $e[]=[array_merge(["DATABASE ".$zn[1]],preg_split($rgex,$ed->utf($buf),-1,PREG_SPLIT_NO_EMPTY))];
	...
	if(!empty($lite)){
		...
		foreach($lt as $k=>$l) echo "SQLite <b>$k</b>: $l<br/>";
	}
	if(!empty($e) && is_array($e)){
		set_error_handler(function(){});
		foreach($e as $ar){
		if(substr($ar[0],0,8)=='DATABASE'){
		$db=substr($ar[0],9);array_shift($ar);
		...
		echo $head.$ed->menu($db)."<div class='col2'><p>Successfully executed: <b>$q query</b></p>$out";
	}
break;
...
case "31"://export form
	...
	if($ex < 1) $ed->redir("5/$db",["err"=>"No export empty DB"]);
	...
	foreach($r_tts as $tts) echo "<option value='$tts'>$tts</option>";
	}
	...
	foreach($ffo as $k=> $ff) echo "<p><input type='radio' name='ffmt[]' onchange='fmt()' value='$k'".($k=='sql' ? ' checked':'')."/> $ff</p>";
...
case "32"://export
	...
	if($ffmt[0]=='sql'){//sql
		...
		$sql[$db.$ffext]=$sq;
		if($ftype=="plain" || count($dbs)<2) $sql=$sql[$db.$ffext];
		}
		...
		if($ftype=="plain" || count($dbs)<2) $sql=$sql[$db.$ed->ext];
		}
	}
